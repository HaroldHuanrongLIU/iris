"""Trajectory metrics for SurgWMBench sparse-anchor and dense-pseudo evaluation."""
...
def _as_batched_coords(value: Any, name: str) -> np.ndarray:
...
def _prepare_pair(pred: ArrayLike, target: ArrayLike, mask: Any | None = None) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    pred_np = _as_batched_coords(pred, "pred")
    target_np = _as_batched_coords(target, "target")
...
def _prepare_coords(coords: ArrayLike, mask: Any | None = None) -> tuple[np.ndarray, np.ndarray]:
    coords_np = _as_batched_coords(coords, "coords")
...
def fde(pred: ArrayLike, target: ArrayLike, mask: Any | None = None) -> float | None:
    ...
    pred_np, target_np, mask_np = _prepare_pair(pred, target, mask)
    values = [float(np.linalg.norm(pred_i[-1] - target_i[-1])) for pred_i, target_i in _valid_pairs(pred_np, target_np, mask_np)]
    return _mean_or_none(values)


def endpoint_error(pred: ArrayLike, target: ArrayLike, mask: Any | None = None) -> float | None:
    """Endpoint error, equivalent to FDE for masked trajectories."""

    return fde(pred, target, mask)


def discrete_frechet(pred: ArrayLike, target: ArrayLike, mask: Any | None = None) -> float | None:
    """Mean discrete Frechet distance over valid trajectories."""

    pred_np, target_np, mask_np = _prepare_pair(pred, target, mask)
    return _mean_or_none([_discrete_frechet_single(pred_i, target_i) for pred_i, target_i in _valid_pairs(pred_np, target_np, mask_np)])
...
def symmetric_hausdorff(pred: ArrayLike, target: ArrayLike, mask: Any | None = None) -> float | None:
    ...
    pred_np, target_np, mask_np = _prepare_pair(pred, target, mask)
    return _mean_or_none([_hausdorff_single(pred_i, target_i) for pred_i, target_i in _valid_pairs(pred_np, target_np, mask_np)])
